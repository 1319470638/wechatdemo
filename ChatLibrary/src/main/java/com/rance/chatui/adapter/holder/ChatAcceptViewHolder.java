...
public class ChatAcceptViewHolder extends BaseViewHolder<MessageInfo> {
    ...
    public void setData(MessageInfo data) {
        ...
        try{
            Drawable drawable = mContext.getResources().getDrawable(Integer.parseInt(data.getHeader()));
            Glide.with(mContext).load("").error(drawable).into(chatItemHeader);
        }catch (Exception e){
            e.printStackTrace();
            Glide.with(mContext).load(data.getHeader()).error(R.mipmap.ic_launcher).into(chatItemHeader);
        }
